		...
		foreach ($Account in $Identity)
		{
			TRY
			{
				IF ($PSBoundParameters['Recurse'])
				{
					...
					ForEach-Object -Process {
						$_.directreports | ForEach-Object -Process {
							...
							Get-ADUser -Identity $PSItem -Properties Enabled, manager | Select-Object -Property GivenName, Surname, SamAccountName, Enabled, @{ Name = "Manager"; Expression = { (Get-Aduser -identity $psitem.manager).samaccountname } }
				...
				IF (-not ($PSBoundParameters['Recurse']))
				{
					...
					Get-ADUser -Properties Enabled, manager | Select-Object -Property GivenName, Surname, SamAccountName, Enabled, @{ Name = "Manager"; Expression = { (Get-Aduser -identity $psitem.manager).samaccountname } }
				}#IF (-not($PSBoundParameters['Recurse']))
			}#TRY
			CATCH
			{
